...
function MSBReader(file: File) {
  ...
  reader.onloadend = () => {
    ...
    for (let currentGroupNumber = 0; currentGroupNumber < groupsTotal; currentGroupNumber++) {
      ...
      model.vertices.push(new Array<number>());
      model.indices.push(new Array<number>());
      model.normals.push(new Array<number>());
      model.uvs.push(new Array<number>());
      model.materials.push(new Array<string>());
